...
const ReadinessButton: React.FC<{ label: string; active: boolean; onClick: () => void; color: string }> = ({ label, active, onClick, color }) => (
  <button
    onClick={onClick}
    className={`flex-1 py-5 px-1 rounded-2xl font-black transition-all border-2 text-base ${active ? `${color} border-white shadow-lg text-white` : 'bg-zinc-900 border-zinc-800 text-zinc-600'
      }`}
  >
    {label}
  </button>
      ...
      <div className="fixed inset-0 bg-black z-[100] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 space-y-4 pt-6 pb-40">
          {selectedSession.exercises.map((se, i) => {
            ...
            return (
              <div key={i} className="bg-zinc-900 border border-zinc-800 p-6 rounded-[2rem] relative overflow-hidden">
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {se.note && (
                      <p className="text-zinc-500 text-xs mt-2 leading-relaxed italic">{se.note}</p>
                    )}
                  </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 pb-12 bg-gradient-to-t from-black via-black to-transparent z-40">
              {isOnboarding ? (
                ...
                <button
                  ...
                  className={`w-full py-6 rounded-[2rem] font-black text-xl shadow-2xl transition-all active:scale-95 cursor-pointer uppercase flex items-center justify-center gap-3 ${saveStatus === 'SAVED' ? 'bg-green-600 text-white' : 'bg-blue-600 text-white'
                    }`}
                >
              ...
              {(editingSession.exercises || []).length === 0 ? (
                ...
                (editingSession.exercises || []).map((se, idx) => (
                  <div key={idx} className="bg-zinc-900 p-5 rounded-[1.8rem] border border-zinc-800 space-y-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[7px] font-black text-zinc-600 uppercase tracking-widest ml-1">Note (optionnel)</label>
                      <textarea
                        value={se.note || ''}
                        onChange={e => {
                          const upd = [...(editingSession.exercises || [])];
                          upd[idx].note = e.target.value;
                          setEditingSession({ ...editingSession, exercises: upd });
                        }}
                        placeholder="Ex: Tempo lent, focus excentrique..."
                        className="w-full bg-black text-zinc-400 text-sm p-3 rounded-lg border border-zinc-800 outline-none focus:border-blue-500 resize-none"
                        rows={2}
                      />
                    </div>
                  </div>
